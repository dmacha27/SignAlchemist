 ...
 * 
 */
const ComparisonSpectrumChart = memo(({ table1, table2, samplingRate, name2, name1 = "Original" }) => {
    const chartRef = useRef(null);
    const draggableRef = useRef(null);

    const dataset1 = useMemo(() => processFFT(table1, samplingRate), [table1, samplingRate]);
    const dataset2 = useMemo(() => processFFT(table2, samplingRate), [table2, samplingRate]);

    const isLargeDataset = dataset1.length > MAX_DATA_LENGTH;

    const minXValue = useMemo(() => Math.min(...dataset1.map(d => d.x)), [dataset1]);
    const maxXValue = useMemo(() => Math.max(...dataset1.map(d => d.x)), [dataset1]);
    const minYValue = useMemo(() => Math.min(...dataset1.map(d => d.y)), [dataset1]);
    const maxYValue = useMemo(() => Math.max(...dataset1.map(d => d.y)), [dataset1]);
    ...
    const handleGoToX = () => {
        if (chartRef.current && goToX !== null && minXValue <= goToX && goToX <= maxXValue) {
    ...
    const handleYMinMax = () => {
        if (chartRef.current && yMin !== null && yMax !== null && yMin <= yMax && minYValue <= yMin && yMax <= maxYValue) {
            handleResetZoom(chartRef.current);
            ...
            {isLargeDataset ? (
                ...
                <div className="flex justify-center items-center gap-4 mt-4">
                    ...
                    <Group spacing="xs">
                        ...
                        />
                        <Button size="xs" onClick={handleGoToX}>Go</Button>
                    </Group>
                    ...
                    <Group spacing="xs">
                        ...
                        />
                        <Button size="xs" onClick={handleYMinMax}>Go</Button>
                    </Group>
                </div>
            )}
        </div>
