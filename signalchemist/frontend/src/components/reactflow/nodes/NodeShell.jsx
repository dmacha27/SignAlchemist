...
const StatusPill = ({ executionState, onClick }) => (
  <Tooltip label={executionState} withArrow position="bottom">
    <button
      type="button"
      onClick={onClick}
      className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/80 text-slate-600 ring-1 ring-slate-200 transition hover:bg-slate-100 dark:bg-gray-950/80 dark:text-slate-200 dark:ring-gray-700 dark:hover:bg-gray-900"
...
export const NodeShell = ({
  ...
  return (
    <div className="min-w-[320px] rounded-[1.45rem] border border-slate-200 bg-white p-3 shadow-[0_20px_50px_rgba(15,23,42,0.12)] dark:border-gray-700 dark:bg-slate-950">
      <div className="rounded-[1.15rem] border border-slate-200 bg-slate-50 p-3.5 dark:border-gray-700 dark:bg-slate-900">
...
NodeShell.defaultProps = {
  ...
  onOutputClick: undefined,
  onDeleteClick: undefined,
  outputTestId: undefined,
...
export const NodeSection = ({ label, children, compact = false }) => (
  <div
    className={`rounded-[1rem] border border-slate-200 bg-white ${compact ? "p-3" : "p-3.5"} dark:border-gray-700 dark:bg-slate-950`}
  >
    {label ? (
...
export const NodeDataTable = ({ headers, rows, emptyMessage }) => {
  if (!rows || rows.length === 0) {
    return (
      <div className="rounded-[1rem] border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-sm text-slate-500 dark:border-gray-700 dark:bg-gray-900 dark:text-slate-400">
        {emptyMessage}
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-[1rem] border border-slate-200 dark:border-gray-700">
      <div className="max-h-[245px] overflow-auto">
        <table className="min-w-full table-auto">
          <thead className="sticky top-0 bg-slate-100 dark:bg-gray-900">
            <tr>
